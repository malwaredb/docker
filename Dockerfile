...
RUN apt-get install -y postgresql-15 postgresql-server-dev-15 libfuzzy2 libfuzzy-dev cmake make build-essential git curl libssl-dev
...
RUN mkdir /malwaredb
RUN mkdir /malwaredb/database
RUN mkdir /malwaredb/samples

# Setup Postgres
RUN /etc/init.d/postgresql start && psql --command "CREATE USER malwaredb WITH PASSWORD 'malwaredb';"
RUN psql --comand "CREATE TABLESPACE malwaredb OWNER malwaredb LOCATION '/malwaredb/database';"
RUN psql --comand "CREATE DATABASE malwaredb OWNER malwaredb TABLESPACE 'malwaredb';"

# Add extensions
RUN psql --comand "CREATE OR REPLACE FUNCTION fuzzy_hash_compare(TEXT, TEXT) RETURNS INTEGER AS 'ssdeep_psql.so', 'pg_fuzzy_hash_compare' LANGUAGE 'c';"
RUN psql --comand "CREATE OR REPLACE FUNCTION tlsh_compare(TEXT, TEXT) RETURNS INTEGER AS 'tlsh_psql.so', 'pg_tlsh_compare' LANGUAGE 'c';"
RUN psql --comand "CREATE OR REPLACE FUNCTION sdhash_compare(TEXT, TEXT) RETURNS INTEGER AS 'sdhash_psql.so', 'pg_sdhash_compare' LANGUAGE 'c';"
RUN psql --comand "CREATE OR REPLACE FUNCTION lzjd_compare(TEXT, TEXT) RETURNS INTEGER AS 'lzjd_psql.so', 'pg_lzjd_compare' LANGUAGE 'c';"
